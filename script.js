...
const clickHandler = () => {
  tops.classList.toggle("open");

  if (isFirstLoad) {
    setTimeout(() => { 
      writeLetter();
    }, 500);
    isFirstLoad = false; // Đánh dấu rằng đã chạy hàm
  }
  setTimeout(() => {
    paper.classList.toggle("show");
    write.classList.toggle("write");
  }, 500);
  setTimeout(() => {
...
// Thêm sự kiện click vào envelope
envelope.addEventListener("click", clickHandler);

// Xóa sự kiện click khỏi envelope
...
const writeLetter = () => {
  ...
  if (sentenceID < sentences.length) {
    envelope.removeEventListener("click", {clickHandler
    });
    const input = sentences[sentenceID];
...
const typewriter = (text, cb = () => { }) => {
  ...
  lineBreak.classList.add("signature");

  }
  if(sentenceID >= sentences.length - 1){
    envelope.addEventListener("click", clickHandler);
  } 
  write.appendChild(lineBreak);
  ...
  for (let i = 0; i < text.length; i++) {
    setTimeout(() => {
      if (text.charAt(i) === '\n') {
        ...
        lineBreak.innerHTML += text.charAt(i);
...
// Nếu bạn muốn chặn cả sự kiện "keydown" khi phím "Ctrl" được nhấn
